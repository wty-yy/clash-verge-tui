...
pub fn sections() -> Vec<Section> {
 ...
 Section{category:1,name:"端口设置",description:"Mixed、SOCKS、HTTP、Redir、TProxy",fields:vec![f("mixed_port","混合代理端口",crate::network::DEFAULT_MIXED_PORT_TEXT,Number),f("socks_port","SOCKS 端口（0 关闭）","0",Number),f("http_port","HTTP 端口（0 关闭）","0",Number),f("redir_port","Redir 端口（0 关闭）","0",Number),f("tproxy_port","TProxy 端口（0 关闭）","0",Number)]},
 Section{category:1,name:"DNS 覆写",description:"解析模式、服务器、Fake IP 与过滤策略",fields:vec![f("dns","启用 DNS 覆写","开启",Toggle),f("dns_listen","监听地址","127.0.0.1:1053",Text),f("dns_mode","增强模式","fake-ip",Choice(&["fake-ip","redir-host"])),f("fake_range","Fake IP 范围","198.18.0.1/16",Text),f("fake_range6","Fake IP IPv6 范围","fc00::/18",Text),f("fake_filter_mode","Fake IP 过滤模式","blacklist",Choice(&["blacklist","whitelist"])),f("dns_ipv6","IPv6 解析","关闭",Toggle),f("prefer_h3","优先 HTTP/3","关闭",Toggle),f("respect_rules","遵循路由规则","关闭",Toggle),f("use_hosts","使用 Hosts","开启",Toggle),f("system_hosts","使用系统 Hosts","开启",Toggle),f("default_dns","默认服务器","223.5.5.5",Text),f("nameserver","域名服务器","https://223.5.5.5/dns-query,https://1.12.12.12/dns-query",Text),f("fallback","回退服务器","https://1.1.1.1/dns-query",Text),f("proxy_dns","代理节点 DNS","https://223.5.5.5/dns-query",Text),f("direct_dns","直连 DNS","https://223.5.5.5/dns-query",Text),f("direct_policy","直连遵循策略","关闭",Toggle),f("fake_filter","Fake IP 过滤","*.lan,localhost",Text),f("dns_policy","域名策略","",Multiline),f("geo_filter","GeoIP 过滤","关闭",Toggle),f("geo_code","GeoIP 国家代码","CN",Text),f("fallback_cidr","回退 IP CIDR","240.0.0.0/4",Text),f("fallback_domain","回退域名","",Text),f("hosts","Hosts 映射","",Multiline)]},
 ...
 Section{category:1,name:"内核与 GeoData",description:"固定 mihomo v1.19.29；GeoData 来源与更新偏好",fields:vec![f("geo_auto","GeoData 自动更新","开启",Toggle),f("geo_interval","更新间隔 / 小时","24",Number),f("geo_source","镜像、MetaCubeX 或 geox-url YAML","镜像",Multiline)]},
