...
pub fn sections() -> Vec<Section> {
    vec![
 Section{category:0,name:"系统代理",description:"代理主机、PAC、绕过与守卫",fields:vec![f("system_proxy","系统代理","关闭",Toggle),f("proxy_host","代理主机","127.0.0.1",Text),f("pac","PAC 模式","关闭",Toggle),f("guard","代理守卫","开启",Toggle),f("guard_interval","守卫间隔 / 秒","30",Number),f("bypass","代理绕过","localhost;127.*;192.168.*;10.*",Text),f("pac_script","PAC 脚本","function FindProxyForURL(url, host) { return 'DIRECT'; }",Multiline)]},
 Section{category:0,name:"虚拟网卡 TUN",description:"协议栈、路由、DNS 劫持与 MTU",fields:vec![f("tun","虚拟网卡模式","关闭",Toggle),f("tun_stack","协议栈","mixed",Choice(&["mixed","system","gvisor"])),f("tun_device","网卡名称","Mihomo",Text),f("auto_route","自动路由","开启",Toggle),f("strict_route","严格路由","关闭",Toggle),f("auto_redirect","自动重定向","开启",Toggle),f("detect_interface","自动选择出口","开启",Toggle),f("dns_hijack","DNS 劫持","any:53",Text),f("mtu","MTU","9000",Number),f("exclude_route","排除网段","192.168.0.0/16",Text)]},
 Section{category:0,name:"启动与服务",description:"开机启动、后台服务与静默启动",fields:vec![f("auto_launch","开机自启","关闭",Toggle),f("silent","静默启动","关闭",Toggle),f("service","后台服务","未安装",Choice(&["未安装","运行中","已停止"])),f("start_script","启动脚本","",Multiline)]},
 ...
 Section{category:1,name:"内核与 GeoData",description:"内核通道、GeoData 来源与更新偏好",fields:vec![f("core_channel","内核通道","Stable",Choice(&["Stable","Alpha"])),f("geo_auto","GeoData 自动更新","开启",Toggle),f("geo_interval","更新间隔 / 小时","24",Number),f("geo_source","GeoData 来源","MetaCubeX",Text)]},
 ...
 Section{category:2,name:"杂项设置",description:"延迟测试、连接清理与日志保留",fields:vec![f("close_connections","切换节点清理连接","关闭",Toggle),f("auto_check","自动检查更新","开启",Toggle),f("enhance","内置增强","开启",Toggle),f("auto_delay","自动延迟检测","关闭",Toggle),f("delay_interval","测速间隔 / 秒","300",Number),f("test_url","测试链接","https://www.gstatic.com/generate_204",Text),f("timeout","测试超时 / 毫秒","5000",Number),f("log_size","日志大小 / MB","10",Number),f("log_count","日志文件数量","7",Number),f("log_clean","自动清理日志","开启",Toggle)]},
 Section{category:3,name:"备份与恢复",description:"本地演示快照；WebDAV 配置表单",fields:vec![f("auto_backup","自动备份","关闭",Toggle),f("webdav_url","WebDAV 地址","https://example.com/dav",Text),f("webdav_user","用户名","",Text),f("webdav_password","密码","",Secret)]},
 ...
 Section{category:4,name:"关于 Clash Verge TUI",description:"v0.1.0 · 界面预览 · GPL-3.0-only",fields:vec![]},
