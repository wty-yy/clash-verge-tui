# clash-verge-tui.wty-yy.top GitHub mirror
...
| --- | --- | --- |
| `MIRROR_DOMAIN` | `clash-verge-tui.wty-yy.top` | Public mirror hostname, used in usage text and the `x-mirror` response header |
...
| Form | Target |
| --- | --- |
| `/https://github.com/<owner>/<repo>/releases/download/<tag>/<asset>` | Compatible with the gh-proxy prefix convention; `--github-proxy https://clash-verge-tui.wty-yy.top` works directly |
...
3. Settings → Domains & Routes → Add → Custom domain with `clash-verge-tui.wty-yy.top` (`wty-yy.top` is hosted on Cloudflare, so DNS records and certificates are created automatically; do not add A/CNAME records manually).
