...
`v0.1.2` 为界面预览版。八个主页面、二级表单与本地交互可用；mihomo、订阅下载、系统代理和 TUN 尚未接入。所有网络数据均为演示数据。
...
默认启用 Vim 导航，底栏显示 `j/k` 上下选择和 `h/l` 左右切换提示；关闭后隐藏 Vim 提示。首页右侧订阅卡片获得焦点后，按 `Enter` 进入订阅管理。选中状态与开关统一使用 `[✓]` / `[ ]`。流量图随终端宽高缩放，保留同一段历史采样范围。
