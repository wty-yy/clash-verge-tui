...
`v0.1.5` 为界面预览版。八个主页面、二级表单与本地交互可用；mihomo、订阅下载、系统代理和 TUN 尚未接入。所有网络数据均为演示数据。
...
| `←` / `→`、`Tab` / `Shift+Tab` | 首页切换左右区域；其他页面切换页内分组 |
| `↑` / `↓`、`j` / `k` | 选择条目；`Enter` 执行 |
| `/`、`Esc` | 搜索、清除筛选或取消弹窗 |
...
默认启用 Vim 导航，底栏显示 `j/k` 上下选择和 `h/l` 左右切换提示；关闭后隐藏 Vim 提示。首页“进入订阅管理”首次点击仅高亮，已高亮时再次点击或按 `Enter` 进入；两次点击没有时间限制。切回左侧后需要重新选中。选中状态与开关统一使用 `[✓]` / `[ ]`。流量图随终端宽高缩放，保留同一段历史采样范围。

2–8 页列表、备份历史和页面跳转采用连续单行布局；设置表单去除字段间多余空行。

左侧导航采用整块高亮和边框，常规窗口每项 3 行，小窗口每项 2 行，边框与留白均可点击。

支持鼠标点击导航、分组、工具按钮和表单；单击列表行选择，400 毫秒内在同一位置附近双击等同 `Enter`，适用于首页快捷控制、列表、设置项和备份历史。恢复备份仍需确认。多行表单使用 `Enter` 换行，`Tab` 切换字段。终端文本选择使用 `Shift` + 鼠标。
