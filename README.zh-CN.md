...
安装脚本会校验发行包发布的 SHA-256，并兼容 Ubuntu 20.04 的 curl 7.68 等旧版本。`--source proxy` 通过项目维护的 Cloudflare Worker 镜像 `gh.wty-yy.top` 下载脚本与归档（源码与部署说明见 [deploy/gh-mirror](deploy/gh-mirror/README.zh-CN.md)），可用 `--github-proxy https://gh-proxy.com` 切换到社区前缀。`~/.local/bin` 不在 `PATH` 时手动添加；设置 `CLASH_VERGE_TUI_VERSION` 可安装指定的已发布版本。
