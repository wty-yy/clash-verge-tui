...
- TUN 开启且工作区未显式设置出口接口时，若主机存在多条默认路由，按最低 metric 在运行时配置写入 `interface-name`（mihomo 网卡监视器取 netlink 顺序中的首条默认路由，多出口主机可能绑定到无外网的上行）；单出口主机保留自动检测，显式出口接口优先，该覆盖只写运行配置、不写回工作区。
...
- systemd `.path` 指令中的绝对路径不能用命令行式双引号包裹；需要使用 systemd 路径转义并通过 `systemd-analyze verify`。监视服务只保留校验私有内核和设置文件能力所需的 `CAP_SETFCAP`、`CAP_DAC_READ_SEARCH`、`CAP_FOWNER` 与 `CAP_CHOWN`。
...
- 前台 TUI 自管内核时按 `q` 打开退出选择：默认选中 `后台运行`，确认后安装（如缺）并启动当前工作区的 systemd 用户服务、写回 `service=运行中`，保持内核与系统代理运行后退出；另一项为 `退出并停止内核`，`Esc` 取消。`Ctrl+C` 仍为立即退出并停止内核。
- 已附加到活动用户服务（`--service start` 后打开界面）时不显示选择，`q` 直接退出并保持服务运行；退出后在终端打印服务名及 `systemctl --user status` / `stop` 提示。
- 转后台失败（例如已保存 TUN 但权限服务过旧）时先停止自管内核、恢复系统代理，再以英文错误退出；不得在后台服务未就绪时报告成功。
...
- README 遵循 `wty-markdown-standards` 技能：简短直接、介绍 → 使用 → 实现细节 → 开发与版本；命令解释尽量写为代码块内注释。
